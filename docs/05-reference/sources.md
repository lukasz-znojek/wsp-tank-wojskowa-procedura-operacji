...
| 2 | Słownik stanów | Materiał źródłowy: stan pozycji mapy `niezaliczone → w toku → zielone → odrzucone` | Ta dokumentacja: cykl życia artefaktu `szkic → do-przegladu → zatwierdzony → opublikowany` oraz `odrzucony` | To **dwa różne pojęcia**, nie warianty tego samego. Oba pozostają i oba są w kontrakcie planu operacji, w dwóch polach: `status` dla cyklu życia artefaktu, `stages[].status` dla stanu pozycji. Trzecią, odrębną rzeczą jest postęp jako liczba wyliczona ze stanów pozycji | [model stanów](../02-architecture/model-stanow.md), [ADR-005](../02-architecture/decyzje/adr-005-stan-pozycji-w-kontrakcie.md) |
